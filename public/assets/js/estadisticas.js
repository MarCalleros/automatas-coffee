document.addEventListener("DOMContentLoaded", () => {
  // Variables globales
  let isLoading = false
  let currentTab = document.querySelector(".tab.active").getAttribute("href").split("=")[1]
...
async function cargarDatos(tab, pagina = 1, periodo = "dia", params = {}) {
  ...
  try {
    ...
    const newUrl = `estadisticas.php?tab=${tab}${tab === "ventas_periodo" ? `&periodo=${periodo}` : ""}&pagina=${pagina}`
...
function aplicarFechasPersonalizadas() {
  ...
  cargarDatos('ventas_periodo', 1, 'personalizado', { fecha_inicio: fechaInicio, fecha_fin: fechaFin });
  
  // Actualizar el título de la tabla
  const tituloVentas = document.querySelector("#ventas_periodo h1");
  if (tituloVentas) {
...
function cambiarPeriodo(periodo) {
  ...
  cargarDatos("ventas_periodo", 1, periodo);
}
  ...
  document.querySelectorAll(".tab").forEach((tab) => {
    tab.addEventListener("click", function (e) {
      ...
      if (["productos_cantidad", "productos_ingresos", "clientes_compras", "clientes_ingresos"].includes(tabName)) {
        cargarGraficas()
      }
    })
  })
  ...
  currentTab = params.get("tab") || "productos_cantidad"
  ...
  if (["productos_cantidad", "productos_ingresos", "clientes_compras", "clientes_ingresos"].includes(currentTab)) {
    cargarGraficas()
  }
})